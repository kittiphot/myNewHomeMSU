...
import { AtmCommentPage } from '../../user/comment/comment/comment'
import { AtmCommentModalPage } from '../../user/comment//modal/modal'
...
export class AtmUserPage {
  ...
  saveScore() {
    if (this.email == null) {
      ...
      let Ref = this.afDatabase.list('score/atm/' + this.key)
      let params = {
        eamil: this.email,
        score: this.score
      }
  ...
  goToCommentPage() {
    let profileModal = this.modalCtrl.create(AtmCommentPage, {
      key: this.key
    })
  ...
  update(key) {
    let profileModal = this.modalCtrl.create(AtmCommentModalPage, {
      key: this.key,
      commentKey: key
    })
    profileModal.present()
  ...
  onSubmit(myform) {
    if (this.email == null) {
      ...
      let Ref = this.afDatabase.list('comment/atm/' + this.key)
      let params = {
        eamil: this.email,
