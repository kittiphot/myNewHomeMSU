...
export class ParkingUserPage {
  ...
  saveScore() {
    if (this.email == null) {
      ...
      let Ref = this.afDatabase.list('score/parking/' + this.key)
      let params = {
        eamil: this.email,
        score: this.score
      }
  ...
  goToCommentPage() {
    let profileModal = this.modalCtrl.create(ParkingCommentPage, {
      key: this.key
    })
  ...
  update(key) {
    let profileModal = this.modalCtrl.create(ParkingCommentModalPage, {
      key: this.key,
      commentKey: key
    })
    profileModal.present()
  ...
  onSubmit(myform) {
    if (this.email == null) {
      ...
      let Ref = this.afDatabase.list('comment/parking/' + this.key)
      let params = {
        eamil: this.email,
